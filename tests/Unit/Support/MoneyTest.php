...
it('format ra chuỗi tiền Việt Nam đúng định dạng', function () {
    expect(Money::fromInt(1_250_000)->format())->toBe('1.250.000 đ')
        ->and(Money::zero()->format())->toBe('0 đ');
});

it('isZero và isAtLeast hoạt động đúng', function () {
    expect(Money::zero()->isZero())->toBeTrue()
        ->and(Money::fromInt(1)->isZero())->toBeFalse()
        ->and(Money::fromInt(100)->isAtLeast(Money::fromInt(100)))->toBeTrue()
        ->and(Money::fromInt(99)->isAtLeast(Money::fromInt(100)))->toBeFalse();
});

it('isLessThan và equals hoạt động đúng', function () {
    expect(Money::fromInt(99)->isLessThan(Money::fromInt(100)))->toBeTrue()
        ->and(Money::fromInt(100)->isLessThan(Money::fromInt(100)))->toBeFalse()
        ->and(Money::fromInt(100)->equals(Money::fromInt(100)))->toBeTrue()
        ->and(Money::fromInt(100)->equals(Money::fromInt(99)))->toBeFalse();
});

it('chuỗi nhiều phép tính liên tiếp ra đúng kết quả', function () {
    $money = Money::fromInt(100_000)->plus(Money::fromInt(50_000))->minus(Money::fromInt(20_000))->times(2);
