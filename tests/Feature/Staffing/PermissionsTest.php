 ...
 */
it('mở bàn / gọi món: owner, manager, staff được, kitchen không', function () {
    expect(User::factory()->owner()->make()->can('open', TableSession::class))->toBeTrue();
    expect(User::factory()->manager()->make()->can('open', TableSession::class))->toBeTrue();
    ...
    expect(User::factory()->manager()->make()->can('create', Order::class))->toBeTrue();
...
it('hủy món đã gửi bếp: chỉ owner và manager, staff bị chặn', function () {
    $item = new OrderItem;

    expect(User::factory()->owner()->make()->can('cancel', $item))->toBeTrue();
    expect(User::factory()->manager()->make()->can('cancel', $item))->toBeTrue();
    expect(User::factory()->staff()->make()->can('cancel', $item))->toBeFalse();
    expect(User::factory()->kitchen()->make()->can('cancel', $item))->toBeFalse();
});

it('giảm giá: owner không giới hạn, manager tối đa 20%, staff không được giảm', function () {
    $session = new TableSession;

    expect(User::factory()->owner()->make()->can('discount', [$session, 100]))->toBeTrue();
    expect(User::factory()->manager()->make()->can('discount', [$session, 20]))->toBeTrue();
    expect(User::factory()->manager()->make()->can('discount', [$session, 21]))->toBeFalse();
    expect(User::factory()->staff()->make()->can('discount', [$session, 0]))->toBeFalse();
});

it('void bill: chỉ owner và manager, staff bị chặn khi làm việc của manager', function () {
    $session = new TableSession;

    expect(User::factory()->owner()->make()->can('void', $session))->toBeTrue();
    expect(User::factory()->manager()->make()->can('void', $session))->toBeTrue();
    expect(User::factory()->staff()->make()->can('void', $session))->toBeFalse();
});

it('thu tiền: owner, manager, staff được, kitchen không', function () {
    expect(User::factory()->owner()->make()->can('create', Payment::class))->toBeTrue();
    expect(User::factory()->manager()->make()->can('create', Payment::class))->toBeTrue();
...
it('đóng ca người khác: chỉ owner và manager, staff bị chặn', function () {
    $owner = User::factory()->owner()->make(['id' => 10]);
    $manager = User::factory()->manager()->make(['id' => 11]);
    $staff = User::factory()->staff()->make(['id' => 12]);
    $shift = new Shift(['opened_by_user_id' => 999]);

    expect($owner->can('close', $shift))->toBeTrue();
    expect($manager->can('close', $shift))->toBeTrue();
    expect($staff->can('close', $shift))->toBeFalse();
});

it('đổi trạng thái món trên KDS: owner, manager, kitchen được, staff không', function () {
    $item = new OrderItem;

    expect(User::factory()->owner()->make()->can('updateStatus', $item))->toBeTrue();
    expect(User::factory()->manager()->make()->can('updateStatus', $item))->toBeTrue();
...
it('xem báo cáo doanh thu: chỉ owner và manager', function () {
    expect(Gate::forUser(User::factory()->owner()->make())->allows('view-revenue-report'))->toBeTrue();
    expect(Gate::forUser(User::factory()->manager()->make())->allows('view-revenue-report'))->toBeTrue();
...
it('xem giá vốn và lợi nhuận: chỉ owner, manager cũng bị chặn', function () {
    expect(Gate::forUser(User::factory()->owner()->make())->allows('view-cost-profit'))->toBeTrue();
    expect(Gate::forUser(User::factory()->manager()->make())->allows('view-cost-profit'))->toBeFalse();
});

it('quản lý món, bàn, nhân viên: chỉ owner và manager', function () {
    foreach ([Product::class, DiningTable::class, User::class] as $lop) {
        expect(User::factory()->owner()->make()->can('viewAny', $lop))->toBeTrue();
        expect(User::factory()->manager()->make()->can('viewAny', $lop))->toBeTrue();
